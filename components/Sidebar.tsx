...
const getNavSections = (t: any, ar: boolean): NavSection[] => [
        ...
        items: [
            {
                ...
                permission: 'crm.dashboard.view.brand',
                children: [
    ...
    },
    {
        ...
        items: [
            { id: 'analytics', icon: 'fa-chart-pie', label: ar ? 'مركز التحليلات' : 'Analytics Hub', permission: 'analytics.dashboard.view.brand' },
        ],
    },
    {
        id: 'ops',
        ...
        items: [
            { id: 'integrations', icon: 'fa-plug', label: ar ? 'التكاملات' : 'Integrations', permission: 'integrations.health.view.brand' },
            { id: 'integration-os', icon: 'fa-network-wired', label: ar ? 'الأصول المرتبطة' : 'Connected Assets', permission: 'integrations.health.view.brand' },
            { id: 'brands-manage', icon: 'fa-layer-group', label: ar ? 'إدارة البراندات' : 'Manage Brands' },
            { id: 'team-management', icon: 'fa-users', label: ar ? 'إدارة الفريق' : 'Team', permission: 'workspace.team.view.own' },
            { id: 'billing', icon: 'fa-credit-card', label: ar ? 'الباقة والاشتراك' : 'Plan & Billing', permission: 'workspace.billing.view.own' },
...
export const Sidebar: React.FC<SidebarProps> = React.memo(({
    ...
    const { hasPermission, isPlatformAdmin, isLoading: permLoading } = usePermissions();
    ...
    const filterItems = (items: NavItem[]): NavItem[] =>
        items
            .filter(item => !item.permission || isPlatformAdmin || hasPermission(item.permission))
            .map(item => item.children ? { ...item, children: filterItems(item.children) } : item);

    const navSections = useMemo(() => {
        const raw = getNavSections(t, ar);
        if (permLoading) return raw;
        return raw
            .map(s => ({ ...s, items: filterItems(s.items) }))
            .filter(s => s.items.length > 0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [t, ar, permLoading, isPlatformAdmin, hasPermission]);
        ...
        return (
            <div key={item.id} className="group">
                <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-2xl transition-all ${isCollapsed && !nested ? 'justify-center px-0 py-3' : ar ? 'px-3 py-3 text-right' : 'px-3 py-3 text-left'
                        } ${isActive
                            ? 'bg-brand-primary text-white shadow-primary-glow'
                            : 'text-light-text-secondary hover:bg-brand-primary/10 hover:text-brand-primary dark:text-dark-text-secondary dark:hover:bg-brand-primary/10 dark:hover:text-brand-primary'
                ...
                {hasChildren && !isCollapsed && (
                    <div className={`grid transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${isMenuOpen ? 'grid-rows-[1fr] opacity-100 mt-2' : 'grid-rows-[0fr] opacity-0'}`}>
    ...
    return (
        <>
            ...
            {isCollapsed && popout && (
                <div
                    className="fixed z-[60] min-w-[220px] overflow-hidden rounded-[1.5rem] border border-light-border/40 bg-white/80 shadow-2xl backdrop-blur-lg dark:border-dark-border/40 dark:bg-slate-900/80 animate-fade-in"
                    style={ar
                        ...
                        {popout.children.map(child => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all duration-200 ${ar ? 'text-right' : 'text-left'
                                        } ${childActive
                                            ? 'bg-brand-primary text-white shadow-primary-glow'
                                            : 'text-light-text hover:bg-brand-primary/5 hover:text-brand-primary dark:text-dark-text dark:hover:bg-brand-primary/10 dark:hover:text-brand-primary'
                                        }`}
                                >
            ...
            {createOpen && createPos && (
                <div
                    className="fixed z-[60] overflow-hidden rounded-[1.5rem] border border-light-border/40 bg-white/80 shadow-2xl backdrop-blur-lg dark:border-dark-border/40 dark:bg-slate-900/80 animate-fade-in"
                ...
                >
                    {CREATE_OPTIONS.map((opt, i) => (
                        <button
                            ...
                            className={`group flex w-full items-center gap-3 px-4 py-3 text-sm transition-all duration-200 hover:bg-brand-primary/5 dark:hover:bg-brand-primary/10 ${ar ? 'text-right' : 'text-left'
                                } ${i < CREATE_OPTIONS.length - 1 ? 'border-b border-light-border/30 dark:border-dark-border/30' : ''}`}
                        >
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-brand-primary/10 transition-transform duration-200 group-hover:scale-110 group-hover:-rotate-3">
                                <i className={`fas ${opt.icon} text-xs text-brand-primary`} />
                            </div>
                            <span className="font-medium text-light-text transition-colors group-hover:text-brand-primary dark:text-dark-text dark:group-hover:text-brand-primary">
