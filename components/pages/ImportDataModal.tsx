...
المطلوب: 5 faqs + 3 complaints + 4 keywords. اجعلها واقعية ومفيدة لبراند "${brandName}".`;
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir={ar ? 'rtl' : 'ltr'}>
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={step === 'configure' || step === 'done' ? onClose : undefined} />
            <div className="relative z-10 w-full max-w-lg rounded-2xl border border-dark-border bg-dark-card shadow-2xl flex flex-col max-h-[85vh]">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {DATE_RANGE_OPTIONS.map(opt => (
                                        <button key={opt.value} onClick={() => setDateRange(opt.value)}
                                            className={`rounded-xl border px-3 py-2.5 text-xs font-semibold transition-all text-start ${
                                                dateRange === opt.value
                        ...
                        <div className="space-y-3">
                            ...
                            {items.map(item => (
                                <button key={item.id} onClick={() => toggleItem(item.id)}
                                    className={`w-full text-start rounded-2xl border p-3.5 transition-all ${
                                        item.approved
                                            ? 'border-brand-primary/30 bg-brand-primary/5'
                                            : 'border-dark-border bg-dark-bg/30 opacity-50'
                                    }`}>
                                    <div className="flex items-start gap-2.5">
                                        <div className={`flex-shrink-0 mt-0.5 w-4 h-4 rounded flex items-center justify-center border ${
                                            item.approved ? 'bg-brand-primary border-brand-primary' : 'border-dark-border'
                                        }`}>
