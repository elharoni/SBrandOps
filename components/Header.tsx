...
export const Header: React.FC<HeaderProps> = React.memo(({
    ...
    const { planName, limits, brandUsagePercent } = usePlanLimits();
            ...
            <div className="flex h-16 items-center justify-between gap-2 sm:gap-3 px-3 sm:px-4 md:px-6">
                <div className="flex min-w-0 items-center gap-1.5 sm:gap-3">
                    <button
                        ...
                        className="flex h-[40px] w-[40px] sm:h-[44px] sm:w-[44px] items-center justify-center rounded-xl text-light-text-secondary transition-colors hover:bg-light-card hover:text-light-text dark:text-dark-text-secondary dark:hover:bg-dark-card dark:hover:text-dark-text lg:hidden"
                    ...
                    <div className="relative min-w-0" ref={dropdownRef}>
                        <button
                            ...
                            className="surface-panel-soft flex min-w-[120px] sm:min-w-[182px] items-center gap-1.5 sm:gap-3 rounded-2xl px-2 sm:px-3 py-1.5 sm:py-2 text-start transition-all hover:shadow-primary-glow max-w-[130px] sm:max-w-none active:scale-95 duration-150"
                            aria-label={ar ? 'تبديل البراند' : 'Switch brand'}
                        >
                            {activeBrand ? (
                                <>
                                    <img src={activeBrand.logoUrl} alt={activeBrand.name} className="h-8 w-8 sm:h-10 sm:w-10 rounded-lg sm:rounded-xl object-cover shadow-sm" />
                                    <div className="min-w-0 flex-1">
                                        <p className="text-[9px] sm:text-[11px] font-semibold uppercase tracking-[0.1em] sm:tracking-[0.18em] text-light-text-secondary dark:text-dark-text-secondary">
                                            {ar ? 'البراند الحالي' : 'Active Brand'}
                                        </p>
                                        <p className="truncate text-xs sm:text-sm font-semibold text-light-text dark:text-dark-text">{activeBrand.name}</p>
                                    </div>
                                </>
                            ) : (
                                <div className="min-w-0 flex-1">
                                    <p className="text-xs sm:text-sm font-semibold text-light-text dark:text-dark-text">{ar ? 'اختر براند' : 'Select Brand'}</p>
                                </div>
                            )}
                            <i className={`fas fa-chevron-down text-[10px] sm:text-[11px] text-light-text-secondary transition-transform dark:text-dark-text-secondary ${isDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>

                        <div className={`absolute top-full z-50 mt-2 w-72 sm:w-80 overflow-hidden rounded-3xl glass-modal shadow-2xl ${ar ? 'right-0' : 'left-0'} max-w-[calc(100vw-2rem)] ${isDropdownOpen ? 'visible opacity-100 translate-y-0' : 'invisible opacity-0 -translate-y-1'} transition-all duration-150`}>
                            <div className="border-b border-light-border/60 dark:border-dark-border/40 px-4 py-3">
                                ...
                                {brands.map((brand) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-start transition-all active:scale-[0.98] duration-150 ${isActive
                            ...
                            <div className="border-t border-light-border/60 dark:border-dark-border/40 p-2 space-y-1">
                                <button
                                    onClick={() => {
                                        onAddBrand();
                                        setIsDropdownOpen(false);
                                    }}
                                    className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-brand-primary/30 px-3 py-3 text-sm font-semibold text-brand-primary transition-all active:scale-[0.98] duration-150 hover:bg-brand-primary hover:text-white"
                                >
                                ...
                                <button
                                    ...
                                    className="flex w-full items-center justify-center gap-2 rounded-2xl px-3 py-2.5 text-sm font-medium text-light-text-secondary transition-all active:scale-[0.98] duration-150 hover:bg-light-bg hover:text-light-text dark:text-dark-text-secondary dark:hover:bg-dark-bg dark:hover:text-dark-text"
                                >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="surface-panel-soft flex items-center gap-1.5 sm:gap-2 rounded-2xl px-1.5 py-1 sm:px-2 sm:py-1.5">
                        <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-brand-primary text-xs sm:text-sm font-bold text-white">
                        ...
                        {onSignOut && (
                            <button
                                onClick={onSignOut}
                                className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-xl text-light-text-secondary transition-all active:scale-90 duration-150 hover:bg-light-bg hover:text-red-500 dark:text-dark-text-secondary dark:hover:bg-dark-bg dark:hover:text-red-300"
