...
const PLATFORM_META: Record<SocialPlatform, { icon: string; color: string }> = {
    ...
    [SocialPlatform.X]:         { icon: 'fab fa-x-twitter',        color: 'text-slate-400' },
...
export const ConnectionIntentModal: React.FC<Props> = ({ isOpen, onClose, onSelectPlatform }) => {
    ...
    return (
        ...
        >
            <div
                className="w-full max-w-lg rounded-3xl border border-black/10 bg-white/70 shadow-2xl shadow-black/10 backdrop-blur-2xl backdrop-saturate-150 dark:border-white/10 dark:bg-slate-900/70"
            ...
            >
                ...
                <div className="flex items-center justify-between border-b border-black/10 px-6 py-4 dark:border-white/10">
                    ...
                    <button
                        onClick={onClose}
                        className="flex h-8 w-8 items-center justify-center rounded-lg text-light-text-secondary hover:bg-black/5 hover:text-light-text dark:hover:bg-white/10 dark:hover:text-dark-text"
                    >
                    ...
                    {!selected ? (
                        ...
                        <>
                            ...
                            <div className={`mb-4 flex items-center gap-3 rounded-xl border p-3 ${selected.bgColor}`}>
                                ...
                                <div>
                                    <p className="text-sm font-bold text-light-text dark:text-dark-text">
                                        {ar ? selected.labelAr : selected.labelEn}
                                    </p>
                                ...
                                <button
                                    ...
                                    className="ms-auto text-xs text-light-text-secondary hover:text-light-text dark:text-dark-text-secondary dark:hover:text-dark-text"
                                >
                            ...
                            {selected.platforms.length > 0 ? (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                        {selected.platforms.map(platform => {
                                            ...
                                            return (
                                                <button
                                                    key={platform}
                                                    onClick={() => handlePlatformPick(platform)}
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        {Object.values(SocialPlatform)
                                            .filter(p => !selected.platforms.includes(p))
                                            .map(platform => {
                                                const meta = PLATFORM_META[platform];
                                                return (
                                                    <button
                                ...
                                <div className="rounded-xl bg-amber-500/10 p-4 text-center">
                                    ...
                                    <p className="mt-1 text-xs text-light-text-secondary dark:text-dark-text-secondary">
                                        {ar
                                            ? 'هذا النوع من الربط قيد التطوير. ستجد المنصات المناسبة له هنا قريبًا.'
                                            : 'This connection type is in development. The right platforms will appear here soon.'}
                                    </p>
                                </div>
                            )}
                        </>
                ...
                <div className="flex justify-end border-t border-black/10 px-6 py-4 dark:border-white/10">
                    <button
                        onClick={onClose}
                        className="rounded-xl border border-black/10 px-4 py-2 text-sm font-medium text-light-text-secondary hover:bg-black/5 dark:border-white/10 dark:text-dark-text-secondary dark:hover:bg-white/5"
                    >
                        {ar ? 'إلغاء' : 'Cancel'}
                    </button>
                </div>
